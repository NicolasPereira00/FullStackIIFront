  ...
  return (
    <div style={{padding:16, display:'grid', gap:12}}>
      ...
      <div style={{display:'flex', gap:8, alignItems:'center'}}>
        ...
        <select onChange={e=>changeStatus(e.target.value)} value="">
          <option value="" disabled>Mudar…</option>
          {STATUS.filter(s => s!==o.status).map(s => <option key={s} value={s}>{s}</option>)}
        </select>
      </div>
      ...
      <div style={{display:'grid', gap:8}}>
        {o.items?.map(it => (
          <div key={it.id} style={{display:'flex', gap:12, borderBottom:'1px solid #eee', padding:'6px 0'}}>
            <div style={{flex:1}}>
              <div><b>{it.product?.name}</b></div>
              <div style={{opacity:.7}}>{it.variant ? `${it.variant.size}/${it.variant.color}` : '—'}</div>
            </div>
      ...
      {o.shippingAddress && (
        <>
          ...
          <div>{o.shippingAddress.street}, {o.shippingAddress.number} {o.shippingAddress.complement ? `- ${o.shippingAddress.complement}` : ''}</div>
          <div>{o.shippingAddress.district} · {o.shippingAddress.city}/{o.shippingAddress.state} · CEP {o.shippingAddress.zip}</div>
        </>
      )}
    </div>
  );
