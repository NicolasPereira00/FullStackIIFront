...
import { ensureCart, addCartItem } from '../api/cart.me';
  ...
  return (
    <div style={{ padding: 16, display: 'grid', gap: 16, gridTemplateColumns: '1fr 1fr' }}>
      <div>
        {hero ? (
          <img
            src={hero}
            alt=""
            style={{ width: '100%', height: 360, objectFit: 'cover', borderRadius: 8, background: '#f2f2f2' }}
          />
        ) : (
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            {p.images.map((img) => (
              <button
                ...
                style={{ border: '1px solid #eee', padding: 0, borderRadius: 6, overflow: 'hidden', width: 72, height: 72 }}
              >
      ...
      <div>
        ...
        <div style={{ opacity: 0.8, marginBottom: 6 }}>{p.brand?.name} · {p.category?.name}</div>
