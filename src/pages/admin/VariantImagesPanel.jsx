...
import { listVariantImages, uploadVariantImage, removeVariantImage } from '../../api/variantImages';

export default function VariantImagesPanel({ variantId }) {
  ...
  async function load() {
    setLoading(true);
    const data = await listVariantImages(variantId);
    setImgs(data);
    setLoading(false);
  }

  useEffect(() => { if (variantId) load(); }, [variantId]);

  async function onPick(e) {
    const f = e.target.files?.[0];
    if (!f) return;
    await uploadVariantImage(variantId, f);
  ...
  async function del(id) {
    ...
    await removeVariantImage(variantId, id);
    await load();
  }
  ...
  return (
    <div style={{display:'grid', gap:8}}>
      <h4>Imagens da variante #{variantId}</h4>
